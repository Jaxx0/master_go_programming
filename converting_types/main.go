...
func main() {
	...
	k := 89989800
	myStr11 := fmt.Sprintf("%d", k)
	fmt.Printf("Type %T and value %d\n ", k, k)
	fmt.Printf("Type %T and value %s\n ", myStr11, myStr11)

	kk := 3.142
	ff := fmt.Sprintf("%f", kk)
	fmt.Printf("Type %T and value %v\n ", kk, kk)
	fmt.Printf("Type %T and value %q\n ", ff, ff)

	var intg int = -7238
	ss := strconv.Itoa(intg)
	fmt.Printf("Type %T and value %q\n ", ss, ss)

	var stng string = "3.142"
	_ = stng
	floatstng, err := strconv.ParseFloat("3.142", 64)
	if err == nil{
		fmt.Printf("Type: %T value %f", floatstng, floatstng)
	}else {
